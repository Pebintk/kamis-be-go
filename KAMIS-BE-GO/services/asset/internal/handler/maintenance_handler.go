...
type MaintenanceHandler struct {
	...
	// assets serves the by-asset history, which AssetService already owns —
	// /api/asset/{platNomor}/maintenance returns the same list.
	assets *service.AssetService
}

func NewMaintenanceHandler(svc *service.MaintenanceService, assets *service.AssetService) *MaintenanceHandler {
	return &MaintenanceHandler{svc: svc, assets: assets}
}

// Create handles POST /api/maintenance/. The trailing slash is load-bearing:
// the legacy controller is mapped at "/api/maintenance/" and the frontend posts
...
// All handles GET /api/maintenance/all.
...
// ByAsset handles GET /api/maintenance/{platNomor}.
func (h *MaintenanceHandler) ByAsset(c *gin.Context) {
	records, err := h.assets.ListMaintenance(c.Request.Context(), c.Param("platNomor"))
	if err != nil {
		httpx.RespondError(c, err)
		return
	}
	httpx.Respond(c, http.StatusOK, "Daftar maintenance untuk asset berhasil diambil", records)
}

// InProgress handles GET /api/maintenance/maintenance-in-progress.
//
...
// Complete handles PATCH /api/maintenance/{id}/complete.
